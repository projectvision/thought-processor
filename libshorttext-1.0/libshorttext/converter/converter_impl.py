...
class TextPreprocessor(object):
	"""
	:class:`TextPreprocessor` is used to pre-preocess the raw texts to a
	...
	be updated even if new tokens are explorered. These new tokens will be
	...
	def get_idx2tok(self, idx):
		...
		.. note::
			...
			mapping. This function should be alawys called with a readonly 
	...
	def save(self, dest_file):
		...
		.. note::
			...
			If the instance attributes are modified 
			(:attr:`stopword_remover`, :attr:`stemmer`, and 
			:attr:`tokenizer`), they will **not** be saved 
			accordingly. Therefore, they need to be set again after
			loaded.
		"""
		self.idx2tok = _dict2list(self.tok2idx)
...
class FeatureGenerator(object):
	"""
	:class:`FeatureGenerator` is used to gernerate uni-gram or bi-gram features.
	"""

	def __init__(self, option='-feature 1', readonly=False):
		#: :attr:`option` is a :class:`str` instnace, which could be
		...
		#: ngram-index mapping. (the defaut value is ``False``).
		self._readonly = readonly
	...
	def toSVM(self, text):
		"""
		Generate a :class:`dict` instance for the given *text*, which a 
		:class:`list` of tokens. Each `key` of the returing dictionary is an
		The index corresponding to a ngram feature, while the
...
class Text2svmConverter(object):
	...
	def toSVM(self, text, class_name = None):
		"""
