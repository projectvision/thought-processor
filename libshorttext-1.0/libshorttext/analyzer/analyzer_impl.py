...
class TextInstance:
	...
	def __init__(self, idx, true_y = '', predicted_y = '', text = '', decvals = None):
...
class InstanceSet:
	...
	def load_text(self):
		...
		i = 0
		for idx, line in enumerate(open(self.filepath, 'r')):
	...
	def _load(self, src, text_src):
		...
		for idx in range(len(true_y)):
			self.insts.append(TextInstance(idx, true_y[idx], predicted_y[idx], None, \
										   list(decvals[idx])))
...
class Analyzer:
	...
	def analyze_single(self, target, amount = 5, output = None):
		'''
		...
		if isinstance(target,str):
			text = target
			true_y = None
			result = predict_single_text(text, self.model)
			decvals = result.decvals
		elif isinstance(target,TextInstance):
			if target.text is None:
				raise Exception('Please load texts first.')
			text, true_y = target.text, target.true_y
			decvals = target.decvals
		if isinstance(output, str):
			output = open(output, 'w')

		features, weights = self.model.get_weight(text)[0:2]
		if not features or not weights:
			raise Exception('Invalid instance.')
		labels = []
		features = [''.join(map(lambda str: str + ' ', feature))[:-1] for feature in features]
		features = features + ['**decval**']
		weights_table = [[0 for i in xrange(len(weights[0]))] for j in xrange(len(features))]
		sorted_idx = [item[0] for item in \
						  sorted(zip(xrange(len(decvals)), decvals), \
									 key=lambda x: x[1], reverse = True)]
		
		for idx in range(len(self.labels)):
			label = self.labels[sorted_idx[idx]]
			labels.append(label)
		for idx_feature, weight_feature in enumerate(weights):
			for idx_label, weight_label in enumerate(weight_feature):
				weights_table[idx_feature][idx_label] = \
					'{0:.3e}'.format(weight_feature[sorted_idx[idx_label]])

		for idx, label in enumerate(labels):
			weights_table[-1][idx] = '{0:.3e}'.format(decvals[sorted_idx[idx]])

		if amount != 0:
			#features = features[:amount]
			labels = labels[:amount]
			#weights_table = weights_table[:amount]
