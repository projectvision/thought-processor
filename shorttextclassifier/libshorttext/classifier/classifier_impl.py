...
class PredictionResult(object):
	...
	:class:`PredictionResult` instances. One can be used for subsquent analysis 
	...
	def __init__(self, text_src = None, model_id = None, true_y = None, predicted_y = None, decvals = None, svm_file = None, labels = None):
	...
	def load(self, file_name):
		...
		for line in fin:
			...
			if line.startswith('version: '):
				...
				raise ValueError("Unexpected argument " + line)
...
class TextModel(object):
	...
	def load(self, model_name):
		""" 
		Load the contents from a :class:`TextModel` direcotry. The 
	...
	def get_weight(self, xi, labels = None):
		"""
		...
		*features* and *labels*. See the following example.
		...
		if isinstance(xi, str):
			xi = self.text_converter.toSVM(xi)
...
def train_converted_text(svm_file, text_converter, grid_arguments = '0', feature_arguments = '', train_arguments = ''):
	"""
	Return a :class:`TextModel` generated from the given LIBSVM-fromat data.
...
def train_text(text_src, svm_file = None, converter_arguments = '', grid_arguments = '0', feature_arguments = '', train_arguments = ''):
	"""
...
def predict_text(text_src, text_model, svm_file=None, predict_arguments=''):
	"""
	...
	:class:`PredictionResult.svm_file`.)
	...
	*predict_arguments* is the arguments for LIBLINEAR predict. Refer to
	...
	return PredictionResult(text_src, text_model._hashcode, true_y, predicted_y, dec_vals, svm_file, text_model.get_labels())


def predict_single_text(text, text_model, predict_arguments = ''):
	"""
	Return an `unanalyzable` :class:`PredictionResult` instance. (Refer to
	:class:`PredictionResult`.) 

	*text* is a short text in :class:`str`. *text* can aslo be a LIBSVM list
	...
	if isinstance(text, str):
		text = text_model.text_converter.toSVM(text)
	elif not isinstance(text, (list, dict)):
